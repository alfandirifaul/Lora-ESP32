...
public:
  ...
  void log(LogLevel level, String category, String message, String data = "")
  {
    ...
    Serial.printf("[%s] %s [%s] %s: %s\n", timestamp.c_str(), levelIcon.c_str(), category.c_str(), levelStr.c_str(), message.c_str());
    if (data.length() > 0)
    {
  ...
  {
    String statusData = "Busy: " + String(busy ? "YES" : "NO") + ", Alarm: " + String(alarm ? "YES" : "NO") + ", Ready: " + String(ready ? "YES" : "NO");
...
void sendReceiverStatus()
{
  ...
  if (success)
  {
    ...
    Serial.printf("   • Busy: %s%s\n", state.isBusy ? "YES" : "NO", (state.isBusy != state.prevBusy) ? " [CHANGED]" : "");
    Serial.printf("   • Alarm: %s%s\n", state.alarmActive ? "YES" : "NO", (state.alarmActive != state.prevAlarmActive) ? " [CHANGED]" : "");
    Serial.printf("   • Ready: %s%s\n", state.isReady ? "YES" : "NO", (state.isReady != state.prevReady) ? " [CHANGED]" : "");
